...
import { blockedDecisionHandoff, normalizeGoalMode, printJson, printStatus } from "./cli-output.js";
import { parseSteeringProposal, printSteerResult } from "./cli-steering.js";
import { goalRuntimeFromArgv, type GoalRuntime } from "./goal-runtime.js";
import { readGrokGoalPlanSnapshot } from "./grok-goal-snapshot.js";
import { buildPlatformGoalInstruction } from "./grok-goal-instruction.js";
...
export async function createGoals(
	...
	const goalRuntime = goalRuntimeFromArgv(argv, readValue);
	const plan = await createUlwLoopPlan(
		repoRoot,
		{
			brief,
			codexGoalMode: normalizeGoalMode(readValue(argv, "--goal-mode") ?? readValue(argv, "--codex-goal-mode")),
			force: hasFlag(argv, "--force"),
		},
		scope,
	);
	if (json) printJson({ ok: true, goalRuntime, plan, summary: summarizeUlwLoopPlan(plan) });
	else {
...
export async function completeGoals(
	...
	const runtime = goalRuntimeFromArgv(argv, readValue);
	const instruction = buildPlatformGoalInstruction({
		plan: result.plan,
		goal: result.goal,
		runtime,
	});
...
export async function checkpoint(
	...
	const goalJsonRaw =
		runtime === "grok"
			? readValue(argv, "--grok-goal-json") ?? readValue(argv, "--codex-goal-json")
			: readValue(argv, "--codex-goal-json") ?? readValue(argv, "--grok-goal-json");
	const goalJson =
		runtime === "grok"
			? await parseGrokGoalJson(statusValue === "complete" ? requiredGoalJson(goalJsonRaw, runtime) : goalJsonRaw)
			: await parseCodexGoalJson(statusValue === "complete" ? requiredGoalJson(goalJsonRaw, runtime) : goalJsonRaw);
	if (statusValue === "complete" && goalJson === undefined) {
		throw new UlwLoopError(
			runtime === "grok" ? "Missing --grok-goal-json." : "Missing --codex-goal-json.",
			runtime === "grok" ? "ULW_LOOP_GROK_GOAL_JSON_REQUIRED" : "ULW_LOOP_CODEX_GOAL_JSON_REQUIRED",
		);
	}
	const qualityGateJson = readValue(argv, "--quality-gate-json");
	const sessionContext = resolveSessionContext(argv, scope);
	const args: CheckpointUlwLoopArgs = {
		...
		...(runtime === "grok"
			? goalJson === undefined
				? {}
				: { grokGoalJson: goalJson }
			: goalJson === undefined
				? {}
				: { codexGoalJson: goalJson }),
...
export async function reviewBlockers(
	...
	const goalJson =
		runtime === "grok"
			? await parseGrokGoalJson(requiredGoalJson(goalJsonRaw, runtime))
			: await parseCodexGoalJson(requiredGoalJson(goalJsonRaw, runtime));
	const sessionContext = resolveSessionContext(argv, scope);
...
export async function grokGoalSnapshotRead(
	repoRoot: string,
	argv: readonly string[],
	json: boolean,
	scope?: UlwLoopScope,
): Promise<number> {
	const sessionId = readValue(argv, "--session-id") ?? scope?.sessionId;
	if (!sessionId) {
		throw new UlwLoopError("Missing --session-id for grok-goal-snapshot read.", "ULW_LOOP_SESSION_REQUIRED");
	}
	const workspaceRoot = readValue(argv, "--workspace-root") ?? repoRoot;
	const snapshot = await readGrokGoalPlanSnapshot(workspaceRoot, sessionId);
	if (json) printJson({ ok: true, snapshot, workspaceRoot, sessionId });
	else {
		process.stdout.write(
			snapshot === null
				? "grok-goal-snapshot: no plan.md found\n"
...
function requiredGoalJson(value: string | undefined, runtime: GoalRuntime): string {
...
function resolveSessionContext(
	...
	return { workspaceRoot: readValue(argv, "--workspace-root") ?? process.cwd(), sessionId };
}
