...
const VALUE_FLAGS = new Set("--brief --brief-file --session-id --codex-goal-mode --goal-mode --goal-runtime --goal --goal-id --criterion-id --status --evidence --notes --codex-goal-json --grok-goal-json --quality-gate-json --kind --rationale --title --objective --target-goal-id --source --after-json --directive-json --directive-file --idempotency-key --workspace-root".split(" "));
...
export async function parseCodexGoalJson(value: string | undefined): Promise<string | undefined> {
	return parsePlatformGoalJson(value, "--codex-goal-json", "ULW_LOOP_CODEX_GOAL_JSON_INVALID");
}

export async function parseGrokGoalJson(value: string | undefined): Promise<string | undefined> {
	return parsePlatformGoalJson(value, "--grok-goal-json", "ULW_LOOP_GROK_GOAL_JSON_INVALID");
}

async function parsePlatformGoalJson(
	...
	try {
		...
		const message = error instanceof Error ? error.message : "unknown error";
		throw new UlwLoopError(`Invalid ${flag}: ${message}`, code, { cause: error });
	}
}
