...
import {
	formatPlatformGoalReconciliation,
	type GoalRuntime,
	readPlatformGoalSnapshot,
	reconcilePlatformGoalSnapshot,
...
export interface CheckpointUlwLoopArgs {
	...
	readonly grokGoalJson?: string;
	readonly goalRuntime?: GoalRuntime;
	readonly qualityGateJson?: string;
	readonly sessionContext?: { workspaceRoot: string; sessionId: string };
}
...
export async function checkpointUlwLoop(
	...
	return withUlwLoopMutationLock(repoRoot, scope, async () => {
		...
		if (args.status === "complete") {
			...
			const runtime: GoalRuntime = args.goalRuntime ?? "grok";
			const goalJson = runtime === "grok" ? args.grokGoalJson ?? args.codexGoalJson : args.codexGoalJson ?? args.grokGoalJson;
			const snapshot = await readPlatformGoalSnapshot(runtime, goalJson, repoRoot, args.sessionContext);
			const reconciliation = reconcilePlatformGoalSnapshot(runtime, snapshot, {
			...
			if (!reconciliation.ok) {
				...
				if (!taskScoped)
					throw new UlwLoopError(
						`${formatPlatformGoalReconciliation(runtime, reconciliation)}${aggregate && snapshot?.status === "complete" && objective !== undefined ? buildTaskScopedAggregateReconciliationHint(goal, final) : ""}`,
						runtime === "grok" ? "ulw_loop_grok_snapshot_mismatch" : "ulw_loop_codex_snapshot_mismatch",
					);
			}
